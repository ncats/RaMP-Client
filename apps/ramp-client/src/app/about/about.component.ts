...
export class AboutComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('briefDescription', { static: false })
  briefDescription!: ElementRef<HTMLElement>;
  ...
  @ViewChild('summaryStatistics', { static: false })
  summaryStatistics!: ElementRef<HTMLElement>;
  ...
  ngAfterViewInit(): void {
    ...
    this.route.queryParams
      .pipe(filter((params) => params.scrollTo))
      .subscribe((params) => {
  ...
  getAnalytesSourceIntersects(): void {
    ...
    this.http
      ...
      .pipe(
        map((response) => {
          Object.keys(response).forEach((key) => {
            response[key].map((intercept: { sets: any[] }) => {
      ...
      .subscribe((response) => {
        ...
        const compounds = response.compounds.sort(
          (a: { sets: string | any[] }, b: { sets: string | any[] }) => {
        ...
        const genes = response.genes.sort(
          (a: { sets: string | any[] }, b: { sets: string | any[] }) => {
            if (a.sets.length < b.sets.length) {
              return -1;
            }
        ...
        for (let i = 0; i < compounds.length; i++) {
          ...
          if (compounds[i].sets.length === 1) {
            ...
            //todo fix error
            // @ts-ignore
            compoundSetNameDict[compoundIntersection.name] =
            ...
            compoundIntersection.setName = '';
            response.compounds[i].sets.forEach((set: string | number) => {
              //todo fix error
              // @ts-ignore
          ...
          if (genes[i].sets.length === 1) {
            ...
            response.genes[i].sets.forEach((set: string | number) => {
              //todo fix error
              // @ts-ignore
  ...
  scrollTo(element: string): void {
    this.elementsDict[element].nativeElement.scrollIntoView({
      behavior: 'smooth',
      block: 'start',
    });
  }
