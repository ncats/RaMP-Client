...
import {
  analyteIdTypeColumns,
  analyteMatchesColumns,
  analyteColumns,
} from './analyte-tables-columns.constant';
...
export abstract class AnalyteQueryBase implements OnInit {
  ...
  ngOnInit(): void {
    this.analyteMatchesDisplayedColumns = this.analyteMatchesColumns.map(
      (item) => item.value
    );
  ...
  loadIdTypes(): void {
    ...
    this.http
      .get<Array<{ analyteType: string; idTypes: string }>>(url)
      .subscribe((response) => {
        this.idTypes = response;
      });
  }

  setSelectedAnalyteMatches(
    analyteMatches: Array<AnalyteMatch>
  ): Array<AnalyteMatch> {
