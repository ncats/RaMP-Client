...
export class OntologiesComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ontologyGroupedOptions!: Array<{
    group: string;
    ontologies: Array<Ontology>;
  }> | null;
  @ViewChild('ontologiesInput', { static: false })
  ontologiesInput!: ElementRef<HTMLInputElement>;
    ...
    const subscription = this.ontologiesInputformControl.valueChanges
      ...
      .subscribe((input) => {
        if (
          ...
          this.selectedOntologies.length === 0 ||
          this.selectedOntologies.indexOf(input) === -1
        ) {
  ...
  loadAutoCompleteOptions(input: any): void {
    ...
    this.http
      .get<Array<Ontology>>(url, options)
      .pipe(
        map((ontologies) => {
          const group = new Map();
      ...
      .subscribe(
        (groupedOntologies) => {
          ...
          if (groupedOntologies.size > 0) {
            this.ontologyGroupedOptions = Array.from(
              groupedOntologies,
              (element) => {
                return {
                  group: element[0],
  ...
  processAutocompleteInputEnd(inputEvent: MatChipInputEvent): void {
    if (this.ontologyGroupedOptions && this.ontologyGroupedOptions.length > 0) {
      for (
        let groupIndex = 0;
        0 < this.ontologyGroupedOptions.length;
        groupIndex++
      ) {
        for (
          let ontologyIndex = 0;
          this.ontologyGroupedOptions[groupIndex].ontologies.length;
          ontologyIndex++
        ) {
          const ontologyName =
            this.ontologyGroupedOptions[groupIndex].ontologies[
              ontologyIndex
            ].Ontology.toLocaleLowerCase();
          if (
            ontologyName === inputEvent.value.toLowerCase() &&
            this.selectedOntologies.indexOf(inputEvent.value) === -1
          ) {
  ...
  submitQuery(): void {
    ...
    if (this.queryType === 'metaFromOnto') {
      ...
      this.identifiersParams = this.identifiersInput
        .toString()
        .split(/\r\n|\r|\n/g);
      //todo fix error
      // @ts-ignore
    ...
    this.http.get<any>(url, options).subscribe(
      ...
      (error) => {
        this.errorMessage =
          'There was a problem processing your request. Please review your input try again.';
        this.loadingService.setLoadingState(false);
      },
      () => {
        this.loadingService.setLoadingState(false);
