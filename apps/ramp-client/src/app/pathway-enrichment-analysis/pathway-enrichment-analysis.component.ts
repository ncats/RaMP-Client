...
export class PathwayEnrichmentAnalysisComponent
  extends AnalyteQueryBase
  implements OnInit
{
  // data for tables
  pathways!: Array<Pathway> | null;
  fisherTestResults!: Array<FisherTestResult> | null;
  filteredFisherTestResults!: Array<FisherTestResult> | null;
  ...
  // clustering
  private clusteringResults!: Array<FisherTestResult> | null;
  ...
  ngOnInit(): void {
    ...
    this.clusteringDisplayedColumns = this.clusteringColumns.map(
      (item) => item.value
    );
    super.ngOnInit();
  }
  ...
  runCombinedFisherTest(): void {
    ...
    this.http
      .post<any>(url, this.pathways)
      .pipe(
        map((response) => {
          if (response.fishresults != null) {
            response.fishresults.forEach(
              (resultsObject: { [x: string]: any }) => {
      ...
      .subscribe(
        (response: {
          ...
          this.fisherTestDisplayedColumns = fisherTestBaseColumns.map(
            (item) => item.value
          );
          if (response.analyte_type && response.analyte_type.length > 0) {
            if (response.analyte_type[0] === 'both') {
              this.fisherTestColumns = this.fisherTestColumns.concat(
                fisherTestMultiTypeColumns
              );
              this.fisherTestDisplayedColumns =
                this.fisherTestDisplayedColumns.concat(
                  fisherTestMultiTypeColumns.map((item) => item.value)
                );
            } else {
              this.fisherTestColumns = this.fisherTestColumns.concat(
                fisherTestSingleTypeColumns
              ...
              this.fisherTestDisplayedColumns =
                this.fisherTestDisplayedColumns.concat(
                  fisherTestSingleTypeColumns.map((item) => item.value)
                );
            }
          }
  ...
  filterFisherTestResults(): void {
    ...
    this.http
      .post<any>(url, this.fisherTestResultsResponse, options)
      .pipe(
        map((response) => {
          if (response.fishresults != null) {
            response.fishresults.forEach(
  ...
  clusterFilteredFisherResults(): void {
    ...
    // @ts-ignore
    // tslint:disable-next-line:no-string-literal
    options.params['perc_analyte_overlap'] = Decimal.div(
      this.percAnalyteOverlap,
      100
    ).toNumber();
    //todo fix error
    // @ts-ignore
    // tslint:disable-next-line:no-string-literal
    options.params['perc_pathway_overlap'] = Decimal.div(
      this.percPathwayOverlap,
      100
    ).toNumber();
    //todo fix error
    // @ts-ignore
    ...
    this.http
      .post<any>(url, this.filteredFisherTestResultsResponse, options)
      .pipe(
        map((response) => {
          if (response.fishresults != null) {
            response.fishresults.forEach(
      ...
      .subscribe(
          ...
          if (response.fishresults && response.fishresults.length > 0) {
            ...
            response.fishresults.map((item) => {
              if (item.Pval != null || item.Pval_combined != null) {
                ...
                //todo fix error
                // @ts-ignore
                item.negativeLogPVal = Decimal.log10(
                  new Decimal(1).dividedBy(pVal)
                ).toNumber();
              }
        ...
        (error) => {
          this.errorMessage =
            'There was a problem processing your request. Please review your input and make sure you entered the correct analyte identifiers.';
          this.loadingService.setLoadingState(false);
        }
      );
  }
  ...
  barClicked(barEvent: any): void {
    const pathways: FisherTestResult[] = [];
    //todo fix error
    // @ts-ignore
  ...
  upSetBarClicked(barEvent: any): void {
    const pathways: (Pathway | undefined)[] = [];
    //todo fix error
    // @ts-ignore
    barEvent.values.forEach((pathwaysourceId) => {
      //todo fix error
      // @ts-ignore
      const pathway = this.pathways.find(
        (_pathway) => _pathway.pathwaysourceId === pathwaysourceId
      );
    ...
    this.dialog.open(TableDialogComponent, {
      data: {
        ...
        tableData: pathways,
        columns: this.pathwayColumns.filter(
          (column) => column.value !== 'commonName'
        ),
      },
      width: '600px',
    });
