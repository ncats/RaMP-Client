...
export class AnalytesFromPathwayComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pathwayMatchesDisplayedColumns = this.pathwayMatchesColumns.map(item => item.value);
    this.pathwayDisplayedColumns = this.pathwayColumns.map(item => item.value);
  ...
  findPathways(): void {
    ...
    this.http.get<any>(url, options)
      .pipe(
        map((response: Array<Pathway>) => {
          ...
          response.forEach(pathway => {
            let index = pathwaysLower.indexOf(pathway.pathwaysourceId.toLowerCase());
            if (index === -1) {
            ...
            if (index > -1) {
              if (pathwayMatches[index].pathwayRampIdList.indexOf(pathway.pathwayRampId) === -1) {
                ...
                } else if (pathwayMatches[index].pathwayName !== pathway.pathwayName) {
                  pathwayMatches[index].pathwayName = 'multiple - click to view';
                }
              } else {
                const existingPathway = pathwayMatches[index].pathways.find(x => x.pathwayRampId === pathway.pathwayRampId);
                if (existingPathway.pathwaySourceList.indexOf(pathway.pathwaysource) === -1) {
                  existingPathway.pathwaySourceList.push(pathway.pathwaysource);
                }
              }
              if (pathwayMatches[index].pathwaySourceList.indexOf(pathway.pathwaysource) === -1) {
                pathwayMatches[index].pathwaySourceList.push(pathway.pathwaysource);
              }
              if (pathwayMatches[index].pathwayCategoryList.indexOf(pathway.pathwayCategory) === -1) {
                pathwayMatches[index].pathwayCategoryList.push(pathway.pathwayCategory);
              }
            }
          });

          pathwayMatches.map(pathwayMatch => {
            pathwayMatch.pathwaySources = pathwayMatch.pathwaySourceList.join(', ');
            pathwayMatch.pathwayCategories = pathwayMatch.pathwayCategoryList.join(', ');
      ...
      .subscribe((response: Array<PathwayMatch>) => {
  ...
  mapPathwaysToAnalytes(): void {
    ...
    this.http.get<any>(url, options)
      ...
      .subscribe((response: Array<Analyte>) => {
  ...
  expandRow(row: PathwayMatch): PathwayMatch {
