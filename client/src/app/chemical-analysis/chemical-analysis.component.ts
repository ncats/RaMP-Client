...
export class ChemicalAnalysisComponent extends AnalyteQueryBase  implements OnInit {
  ...
  ngOnInit(): void {
    super.ngOnInit();
    this.selectedProperties = this.availableChemicalProperties.map(item => item.value);
  }
  ...
  findAnalytes(): void {
    ...
    this.analyteService.findAnalytes(analytes, 'compound').subscribe((response: Array<AnalyteMatch>) => {
      ...
      if (item.analytes && item.analytes.length > 0) {
        item.analytes.forEach(analyte => {
          if (analyte.isSelected) {
            this.metabolitesParameters = this.metabolitesParameters.concat(analyte.sourceIdsList);
          }
        });
      }
    });
    ...
    this.http.get<any>(url, options)
      ...
      .subscribe((response: Array<any>) => {
        ...
        this.chemicalProperties.map(prop => {
          prop.input = this.analyteMatches.find(match => match.sourceIdsList.indexOf(prop.chem_source_id) > -1).input;
          return prop;
        });
        // tslint:disable-next-line:no-string-literal
        this.missedQueryElements = response['query_report'] && response['query_report']['missed_query_elements'];
        this.loadingService.setLoadingState(false);
  ...
  setColumns(): void {
    ...
    if (this.selectedProperties.length === 0) {
      ...
      const selectedColumns = this.dynamicColumns.filter(column => this.selectedProperties.indexOf(columnsToChemprops[column.value]) > -1);
